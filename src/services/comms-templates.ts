...
export const COMMS_TEMPLATES: CommsTemplate[] = [
  ...
  // ── Transactional EMAIL (added for the Production Transactional Email Platform) ──
  { key: 'email_otp', channel: 'email', category: 'auth', vars: ['code'],
    subjectAr: 'رمز تأكيد هات الآن', subjectEn: 'Your HAAT NOW verification code',
    bodyAr: 'رمز التأكيد الخاص بك هو {{code}}. صالح لـ 5 دقائق. لا تُشاركه مع أحد.',
    bodyEn: 'Your verification code is {{code}}. Valid for 5 minutes. Do not share it with anyone.' },
  { key: 'account_verification', channel: 'email', category: 'auth', vars: ['name', 'link'],
    subjectAr: 'فعّل حسابك في هات الآن', subjectEn: 'Verify your HAAT NOW account',
    bodyAr: 'مرحباً {{name}}، فعّل حسابك عبر الرابط: {{link}}',
    bodyEn: 'Hi {{name}}, verify your account using this link: {{link}}' },
  { key: 'password_reset', channel: 'email', category: 'auth', vars: ['name', 'link'],
    subjectAr: 'إعادة تعيين كلمة المرور', subjectEn: 'Reset your password',
    bodyAr: 'مرحباً {{name}}، لإعادة تعيين كلمة المرور استخدم الرابط: {{link}}. تجاهل الرسالة إن لم تطلبها.',
    bodyEn: 'Hi {{name}}, reset your password using this link: {{link}}. Ignore this email if you did not request it.' },
  { key: 'security_alert', channel: 'email', category: 'auth', vars: ['event', 'when', 'device'],
    subjectAr: 'تنبيه أمني على حسابك', subjectEn: 'Security alert on your account',
    bodyAr: 'رصدنا {{event}} في {{when}} من {{device}}. إن لم يكن أنت، أمّن حسابك فوراً.',
    bodyEn: 'We detected {{event}} on {{when}} from {{device}}. If this was not you, secure your account immediately.' },
  { key: 'payment_confirmation', channel: 'email', category: 'transactional', vars: ['orderId', 'amount', 'method'],
    subjectAr: 'تأكيد الدفع — طلب #{{orderId}}', subjectEn: 'Payment confirmed — order #{{orderId}}',
    bodyAr: 'تم تأكيد دفع {{amount}} عبر {{method}} لطلبك #{{orderId}}. شكراً لك.',
    bodyEn: 'Your payment of {{amount}} via {{method}} for order #{{orderId}} is confirmed. Thank you.' },
  { key: 'driver_arrived', channel: 'email', category: 'transactional', vars: ['orderId', 'captain'],
    subjectAr: 'الكابتن وصل — طلب #{{orderId}}', subjectEn: 'Your captain has arrived — order #{{orderId}}',
    bodyAr: 'وصل الكابتن {{captain}} لتسليم طلبك #{{orderId}}.',
    bodyEn: 'Your captain {{captain}} has arrived to deliver order #{{orderId}}.' },
  { key: 'delivery_completed', channel: 'email', category: 'transactional', vars: ['orderId', 'total'],
    subjectAr: 'تم توصيل طلبك #{{orderId}}', subjectEn: 'Your order #{{orderId}} was delivered',
    bodyAr: 'تم توصيل طلبك #{{orderId}} بإجمالي {{total}}. نتمنى لك وجبة شهية!',
    bodyEn: 'Your order #{{orderId}} ({{total}}) was delivered. Enjoy your meal!' },
  { key: 'affiliate_approved', channel: 'email', category: 'merchant', vars: ['name', 'code'],
    subjectAr: 'تم اعتماد حساب التسويق بالعمولة', subjectEn: 'Your affiliate account is approved',
    bodyAr: 'مرحباً {{name}}، تم اعتماد حسابك. رمز الإحالة الخاص بك: {{code}}.',
    bodyEn: 'Hi {{name}}, your affiliate account is approved. Your referral code: {{code}}.' },
];
