...
export const growthService = {
  ...
  // Referral qualification is SERVER-DRIVEN (N-2 remediation): the `qualify_referral` RPC is
  // service-only and runs automatically — and only — when the referee's order becomes both PAID
  // and DELIVERED (trigger `qualify_referral_on_delivery`), with server-controlled rewards.
  // The client no longer calls the RPC; this is a safe no-op kept for call-site stability.
  async qualifyReferral(_customerId: string, _orderId: string): Promise<{ error: any }> {
    return { error: null };
  },
