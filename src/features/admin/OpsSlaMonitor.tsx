...
const SLA_MINUTES = 45;                                   // active-order SLA threshold
const ACTIVE = ['pending', 'confirmed', 'preparing', 'delivering'];
const FAILED = ['cancelled', 'failed', 'rejected'];
...
export const OpsSlaMonitor: React.FC = () => {
  ...
  const delayed = active.map(o => ({ ...o, _age: ageMin(o) })).filter(o => o._age !== null && o._age > SLA_MINUTES).sort((a, b) => (b._age || 0) - (a._age || 0));
  ...
  return (
    <div className="space-y-4" id="ops_sla_monitor" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <MetricCard label={L('طلبات متأخرة', 'Delayed orders')} value={delayed.length} Icon={Timer} accent={delayed.length ? '#f87171' : undefined} hint={L(`> ${SLA_MINUTES} دقيقة`, `> ${SLA_MINUTES} min`)} />
