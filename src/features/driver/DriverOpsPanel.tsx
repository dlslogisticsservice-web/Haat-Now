...
export const DriverOpsPanel: React.FC<{ driverId: string }> = ({ driverId }) => {
  ...
  const respond = async (assignmentId: string, accept: boolean) => {
    ...
    if (data === 'lost') alert('تم قبول الطلب من مندوب آخر.');
    await load();
  };
  const requestPayout = async () => {
    const raw = prompt(`الرصيد المتاح: ${money(wallet.available)}\nأدخل مبلغ السحب:`);
    if (!raw) return;
    ...
    setBusy(false);
    if (error) return alert(error.message);
    alert('تم إرسال طلب السحب 🟢'); await load();
  };

  return (
    <div className="space-y-4" id="driver_ops_panel" dir="rtl">
      {/* Shift / attendance */}
      <Card variant="z3" radius="xl" padding="p-5">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-headline-sm font-semibold text-[var(--color-on-surface)]">الوردية</h3>
          <Badge variant={shift ? (onBreak ? 'secondary' : 'success') : 'secondary'}>
            {shift ? (onBreak ? 'في استراحة' : 'وردية نشطة') : 'خارج الوردية'}
          </Badge>
        </div>
        <div className="flex gap-2 flex-wrap">
          {!shift ? (
            <Button size="sm" loading={busy} onClick={startShift}>بدء الوردية</Button>
          ) : (
            <>
              <Button size="sm" variant="secondary" loading={busy} onClick={toggleBreak}>{onBreak ? 'إنهاء الاستراحة' : 'استراحة'}</Button>
              <Button size="sm" variant="secondary" loading={busy} onClick={endShift}>إنهاء الوردية</Button>
            </>
          )}
        </div>
        {shift?.actual_start && (
          <p className="text-label-sm mt-2" style={{ color: 'var(--color-on-surface-variant)' }}>
            بدأت: {new Date(shift.actual_start).toLocaleString('ar')}
          </p>
        )}
      </Card>
      ...
      {offers.length > 0 && (
        <Card variant="z4" radius="xl" padding="p-5">
          <h3 className="text-headline-sm font-semibold text-[var(--color-on-surface)] mb-3">عروض جديدة ({offers.length})</h3>
          <div className="space-y-3">
            {offers.map(o => (
              <div key={o.id} className="flex items-center justify-between gap-3 flex-wrap pb-3 border-b border-[rgba(255,255,255,0.06)] last:border-0 last:pb-0">
                <div>
                  <p className="font-bold text-[var(--color-on-surface)]">{o.orders?.merchant_branches?.name ?? 'طلب'} · {money(o.orders?.delivery_fee ?? 0)}</p>
                  <p className="text-label-sm" style={{ color: 'var(--color-on-surface-variant)' }}>
                    {o.orders?.merchant_branches?.zones?.name ?? ''}{o.distance_km != null ? ` · ${o.distance_km} كم` : ''}
                  </p>
                </div>
                <div className="flex gap-2">
                  <Button size="sm" loading={busy} onClick={() => respond(o.id, true)}>قبول</Button>
                  <Button size="sm" variant="secondary" loading={busy} onClick={() => respond(o.id, false)}>رفض</Button>
                </div>
              </div>
            ))}
          </div>
      ...
      <Card variant="z3" radius="xl" padding="p-5">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-headline-sm font-semibold text-[var(--color-on-surface)]">المحفظة</h3>
          <Button size="sm" loading={busy} onClick={requestPayout} disabled={wallet.available <= 0}>طلب سحب</Button>
        </div>
        <div className="grid grid-cols-3 gap-3 text-center">
          <div>
            <p className="text-headline-sm font-bold" style={{ color: 'var(--color-lime-vb, #9ed442)' }}>{wallet.available.toFixed(0)}</p>
            <p className="text-label-sm" style={{ color: 'var(--color-on-surface-variant)' }}>متاح</p>
          </div>
          <div>
            <p className="text-headline-sm font-bold text-[var(--color-on-surface)]">{wallet.pending.toFixed(0)}</p>
            <p className="text-label-sm" style={{ color: 'var(--color-on-surface-variant)' }}>قيد الانتظار</p>
          </div>
          <div>
            <p className="text-headline-sm font-bold text-[var(--color-on-surface)]">{wallet.lifetime.toFixed(0)}</p>
            <p className="text-label-sm" style={{ color: 'var(--color-on-surface-variant)' }}>الإجمالي</p>
          </div>
        </div>
      </Card>
    </div>
